...
% in frekventistično
model RF pa z uporabo funkcije \texttt{randomForest} iz istoimenskega paketa za programski jezik R.
...
\begin{itemize} 
  ...
  \item Leta 2013 so se cene začele pri 13 USD in se do konca tega leta zvišale na 770 USD.
...
Na začetku trgovanja imamo poljubno končno količino denarja, s katerim lahko trgujemo. Predpostavimo, da naše trgovanje ne bo tako veliko, da bi vplivali na ceno.
...
\caption{Prikaz časovne vrste $x$ in časovne vrste $y$, za \Sexpr{train.time0} dni s časom $t_0$ = \Sexpr{t1t1}. Dodani sta regresijski premici, dobljeni z Bayesovo linearno regresijo z neinformativnim (uniformnim) apriornim mnenjem za $\beta$ in $\sigma$. \\(Model: $x_i|\beta, \sigma \sim_\text{iid} N(\beta^\intercal t_i, \sigma)$ za časovno vrsto $x$ oziroma $y_i|\beta, \sigma \sim_\text{iid} N(\beta^\intercal t_i, \sigma)$ za časovno vrsto $y$, kjer je $t_i$ $i$-ti dan.)}
...
Model zapišemo kot: 
$$ z_t|\alpha,\beta_1,\beta_2,\beta_3 \sim \textnormal{Bernoulli}(\textnormal{logit}^{-1}(\alpha + \beta_1 w_1 + \beta_2 w_2 + \beta_3 w_3)),$$
...
\end{tabular}
\begin{tablenotes}
\item[$\star$] \small{Pri nekaterih mejah zaupanja $\gamma$ imamo identično trgovanje na učni množici tudi za MA(1) s parametrom $\delta=80$. Poleg tega so nekatere kombinacije parametrov z modeli AR(2), MA(2), ARMA(1,1), ARMA(1,2) in ARMA(2,1) identično trgovale. Ker je model MA(1) bolj enostaven in imamo pri $\delta=100$ za največji nabor zaupanja $\gamma$ najvišje dobičke, izberemo MA(1), $\delta=100$. 
...
###
# results=tex
# print(xtable(div.table.lr,digits=4))
###
# xtable(div.table.ma1,digits=4)
...
\begin{tabular}{|r|r|}
 ...
 parameter & MA(1): $\delta=100$ \\ 
  \hline
$\mu$ & \Sexpr{format(div.table.ma1[1],nsmall=4)} \\ 
  $\theta$ & \Sexpr{format(div.table.ma1[2],nsmall=4)} \\ 
  $\sigma$ & \Sexpr{format(div.table.ma1[3],nsmall=4)} \\ 
  % $\hat{y}$ & 0.8178 \\ 
   \hline
\end{tabular}
\quad
\begin{tabular}{|r|l|}
 ...
 parameter & LR: $D=110, \delta=50$\\ 
  \hline
$\alpha$ & \Sexpr{format(div.table.lr[1],nsmall=4)} \\ 
  $\beta_1$ & \Sexpr{format(div.table.lr[2],nsmall=4)} \\ 
  $\beta_2$ & \Sexpr{format(div.table.lr[3],nsmall=4)} \\ 
  $\beta_3$ & \Sexpr{format(div.table.lr[4],nsmall=4)} \\ 
   \hline
\end{tabular}
...
\begin{figure}[!ht]
\centering %out.width="\\linewidth" #, height=6, width=8>>=
\subcaptionbox{Traceplot.\label{test1a:a}}{\includegraphics[width=0.48\textwidth]{./delta80_d30_ARMA12/mcmc_trace.pdf}}\hspace{0.5em}%
\subcaptionbox{Avtokorelacije.\label{test1a:b}}{\includegraphics[width=0.48\textwidth]{./delta80_d30_ARMA12/mcmc_acf.pdf}}
...
\begin{figure}[!ht]
\centering %out.width="\\linewidth" #, height=6, width=8>>=
\subcaptionbox{Razsevni grafikoni.\label{test1b:a}}{\includegraphics[width=0.48\textwidth]{./delta80_d30_ARMA12/pairs.png}}\hspace{0.5em}%
\subcaptionbox{Aposteriorne porazdelitve parametrov in napovedi.\label{test1b:b}}{\includegraphics[width=0.48\textwidth]{./delta80_d30_ARMA12/mcmc_posterior.pdf}}
...
\begin{figure}[!ht]
\centering %out.width="\\linewidth" #, height=6, width=8>>=
\subcaptionbox{Traceplot.\label{test2a:a}}{\includegraphics[width=0.48\textwidth]{./delta99_d30_ARMA12/mcmc_trace.pdf}}\hspace{0.5em}%
\subcaptionbox{Avtokorelacije.\label{test2a:b}}{\includegraphics[width=0.48\textwidth]{./delta99_d30_ARMA12/mcmc_acf.pdf}}
...
\begin{figure}[!ht]
\centering %out.width="\\linewidth" #, height=6, width=8>>=
\subcaptionbox{Razsevni grafikoni.\label{test2b:a}}{\includegraphics[width=0.48\textwidth]{./delta99_d30_ARMA12/pairs.png}}\hspace{0.5em}%
\subcaptionbox{Aposteriorne porazdelitve parametrov in napovedi.\label{test2b:b}}{\includegraphics[width=0.48\textwidth]{./delta99_d30_ARMA12/mcmc_posterior.pdf}}
...
\begin{figure}[!ht]
\centering %out.width="\\linewidth" #, height=6, width=8>>=
\subcaptionbox{Traceplot.\label{test3a:a}}{\includegraphics[width=0.48\textwidth]{./delta80_d100_ARMA12/mcmc_trace.pdf}}\hspace{0.5em}%
\subcaptionbox{Avtokorelacije.\label{test3a:b}}{\includegraphics[width=0.48\textwidth]{./delta80_d100_ARMA12/mcmc_acf.pdf}}
...
\begin{figure}[!ht]
\centering %out.width="\\linewidth" #, height=6, width=8>>=
\subcaptionbox{Razsevni grafikoni.\label{test3b:a}}{\includegraphics[width=0.48\textwidth]{./delta80_d100_ARMA12/pairs.png}}\hspace{0.5em}%
\subcaptionbox{Aposteriorne porazdelitve parametrov in napovedi.\label{test3b:b}}{\includegraphics[width=0.48\textwidth]{./delta80_d100_ARMA12/mcmc_posterior.pdf}}
...
\begin{figure}[!ht]
\centering %out.width="\\linewidth" #, height=6, width=8>>=
\subcaptionbox{Traceplot.\label{test4a:a}}{\includegraphics[width=0.48\textwidth]{./delta99_d100_ARMA12/mcmc_trace.pdf}}\hspace{0.5em}%
\subcaptionbox{Avtokorelacije.\label{test4a:b}}{\includegraphics[width=0.48\textwidth]{./delta99_d100_ARMA12/mcmc_acf.pdf}}
...
\begin{figure}[!ht]
\centering %out.width="\\linewidth" #, height=6, width=8>>=
\subcaptionbox{Razsevni grafikoni.\label{test4b:a}}{\includegraphics[width=0.48\textwidth]{./delta99_d100_ARMA12/pairs.png}}\hspace{0.5em}%
\subcaptionbox{Aposteriorne porazdelitve parametrov in napovedi.\label{test4b:b}}{\includegraphics[width=0.48\textwidth]{./delta99_d100_ARMA12/mcmc_posterior.pdf}}
